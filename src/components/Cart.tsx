...
function CartItem({ Form, product }: CartItemProps) {
  return (
    <div class="flex flex-row space-x-4">
      ...
      <div class="w-40">{product.title}</div>
      <Form>
        <input type="hidden" name="id" value={product.id} />
        <button type="submit" class="text-red-800 font-bold">
          X
        </button>
      </Form>
    </div>
...
export default function Cart() {
  ...
  const products = createServerData$(getCart, { key: ['cart'] });
      ...
      <Show when={isOpen()}>
        <div class="absolute right-10 mt-4 bg-white p-6 border-gray-500 shadow-lg space-y-4">
          <Suspense fallback={<p>Un instant...</p>}>
            <For each={products()}>
              {(product) => <CartItem Form={Form} product={product} />}
            </For>
          </Suspense>
        </div>
      </Show>
